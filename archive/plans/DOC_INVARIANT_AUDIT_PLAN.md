...
>
> **Why withdraw the planned guard-test infrastructure.** The originally-planned chunk 8 (guard test suite) and chunk 9 (PreToolUse hook) were CI scaffolding for ~10 doc-drift patterns. Most of those patterns are already covered by stronger mechanisms: Rust's compile-time enforcement protects retired identifiers (`parse_legendary`, `legendary_*`); the `serde_breaking_change_on_sprite_shape` test (`compiler/src/ir/mod.rs`) protects the §F4 `img_data`/`sprite_name` retirement; CLAUDE.md `## Working principles` § "Plans" + the retirement-discipline rule protect against §F-N / `§Chunk Na` doc drift. The CI scaffolding's marginal value over those mechanisms doesn't justify the engineering scope.
