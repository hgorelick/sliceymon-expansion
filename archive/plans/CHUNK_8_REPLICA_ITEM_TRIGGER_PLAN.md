...
**Corpus-wide itempool shape (4 working mods).** Non-summon entries (TMs, accessories, consumables, ritemx refs, base-game refs, keyword/template compositions) are the dominant shape by every count that matters: sliceymon carries 16 `itempool.` pools + 23 summon envelopes + many non-summon entries; pansaer has 11 pools and **0 summons**; punpuns has 4 pools and **0 summons**; community has 24 pools and **0 summons**. 3 of 4 mods are entirely non-summon. The `SummonTrigger` IR shipped in §3.2 is a sliceymon-specific specialization; the `NonSummonEntry` typed sum shipped in 8A.5 (`plans/nonsummon-typed-schema.md` §3.1, 15 evidenced variants + `Unclassified` permissive hatch) covers the base case. §1.1's remainder walks the sliceymon summon bytes in detail because those are the bytes the new `SummonTrigger` / `DiceLocation` discriminators must classify; the per-mod non-summon enumeration is in 8A.5 §1.3.
...
Tests that exercise corpus round-trip (T1–T5a, T22–T23a) live inline in-module where extract/build are called; authoring builder tests (T24–T26) live in-module next to the builder. No test needs to be renamed — the `::` prefix is only a reading aid.
...
- [x] The §3.1 retirement set is enforced by the Rust type system — every exhaustive match against `ModifierType` and the new `ReplicaItem` shape would fail to compile if any retired identifier were reintroduced. No grep test is shipped; the type-system enforcement is sufficient.
...
**Tests in 8a**: T24–T26 (authoring builder compile-guards), plus `#[cfg(test)]` round-trip of the struct via serde. The §3.1 retirement set is enforced by the Rust type system (see "Retirements verified" in §5).
