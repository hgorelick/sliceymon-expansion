...
- `compiler/tests/retirements.rs` — already shipped by 8A with T12/T12a/T13/T14 and extended by 8A.5 (`rg -c 'NonSummon\s*\{\s*content\s*:\s*String' compiler/` must be 0). 8B adds **no** new retirement greps; the source-vs-IR behavioral proofs live in the inline tests in `replica_item_parser.rs`, not here.

**Why not duplicate tests from 8A or 8A.5**: retirement greps (T12-T14, T12a) ship in 8A; the per-NonSummon-variant round-trip / distinctness / serde / authoring-exclusion tests (T30.0–T30.15, T31, T32, T34) and Phase-A walker (T33) ship in 8A.5. 8B's tests all target the real-summon-prefilter behavior + the xref Finding widening. No overlap with upstream chunks.
