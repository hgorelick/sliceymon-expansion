...
**Files**: `SPEC.md`, `compiler/tests/doc_invariants_carveouts.toml` (append carve-outs). Chunk-3's PR also amended `compiler/tests/doc_invariants_carveouts_parses.rs`, §3.5, §5.1, and chunk-2 §4.2 — the user's "no deferred correctness" decision pulled the addressing-contract redesign (line-number-keyed → per-file `pattern`-uniqueness) into this chunk's PR rather than leaving the registry on a fragile contract until the next chunk.
