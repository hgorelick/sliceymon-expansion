...
- An entry in the carve-out registry (path picked at implementation start per §6 decision 3) with the path, pattern, rationale, and invariant-not-violated fields documented in the scaffold's schema.

**Registry format** (resolved during round-1 audit; line-number-keyed addressing dropped in PR #18 round-2 per the user's "line numbers are extremely fragile" decision). The registry is **TOML** at `compiler/tests/doc_invariants_carveouts.toml`. The schema is documented by construction in the scaffold's header comment (the example `[[carveout]]` block at the top of the file IS the schema, with field meanings inline as comments above each field, plus the append-discipline bullets at the bottom). The plan does not re-list the field set; the scaffold is the implementation form and the canonical authority.
