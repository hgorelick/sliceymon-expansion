...
| `ModifierType::Legendary` retired in chunk-8A | `rg -in '\b(Legendary\|Capture)\b' personas/ SPEC.md CLAUDE.md` (every hit at `personas/slice-and-dice-design.md` and at SPEC.md's §10 glossary "Capturable / Legendary" entry is a registry-carve-out per §3.5; no pre-grep filter applies, the row's grep runs as-is) |
...
| `xref` IS the validator — no separate pass to bolt on later | `rg -in '\bvalidator pass\b\|no separate validator' personas/ SPEC.md CLAUDE.md` (every hit is a registry-carve-out per §3.5; sites: `SPEC.md` (already registered with pattern `validator pass`), `personas/architecture.md` (chunk 5), `personas/ai-development.md` (chunk 7d — has two row-7 hits in this file, so each entry must lengthen its `pattern` past the bare `validator pass` substring to satisfy per-file uniqueness per the scaffold's append-discipline)) |
...
- An entry in the carve-out registry (path picked at implementation start per §6 decision 3) conforming to the scaffold's schema-by-construction (the example `[[carveout]]` block at the top of `compiler/tests/doc_invariants_carveouts.toml` is the canonical authority for field names).
...
- "Negation, deliberate" — the word being negated is the very thing the grep matches. Canonical site: `SPEC.md` "There is no separate validator pass to bolt on later." (registered; pattern `validator pass` is unique in SPEC.md). Sibling sites: every hit of §3.2 row 7's grep. Each gets its own `[[carveout]]` entry; the rationale text is identical, the `path` and `pattern` fields differ (each `pattern` must satisfy the per-file uniqueness contract documented in the scaffold). Note for chunk 7d: `personas/ai-development.md` has two row-7 hits (one in the "Cross-IR semantic checks" table cell, one in the "Structural validity" bullet that uses the word "structural"), so the bare substring `validator pass` collides on uniqueness — both entries must lengthen their `pattern` past `validator pass` to disambiguate per the scaffold's append-discipline.
...
**Scope**: Create `compiler/tests/doc_invariants_carveouts.toml` as an empty-but-parseable TOML file (zero `[[carveout]]` entries, header comment only), plus a sibling `compiler/tests/doc_invariants_carveouts_parses.rs` well-formedness gate that enforces the registry's structural-and-addressing contract (the canonical enumeration is the gate's module docstring at `compiler/tests/doc_invariants_carveouts_parses.rs`; the load-bearing addressing contract is per-file `pattern`-uniqueness keyed on the canonical path, per §3.5).
...
**Dependencies**: None.
**Consumer**: Chunks 3–7 append entries during their fix decisions; chunk 8's cross-grep guard tests deserialize the registry via `serde::Deserialize` and rely on the well-formedness gate having already enforced the contract so `filter_carveouts` can treat the registry as known-sound (per §5.1).
...
- `~/.cargo/bin/cargo test --test doc_invariants_carveouts_parses` passes against the empty-but-parseable scaffold AND against any chunks 3–7 append (the gate's per-entry assertions are empty-vacuously-true at chunk 2 and load-bearing once chunks 3–7 add entries).
...
- [ ] Every appended `[[carveout]]` entry parses as a `[[carveout]]` table conforming to the scaffold's schema-by-construction (per §3.5), and each entry's `pattern` appears **exactly once** in its `path` (the load-bearing addressing contract; non-uniqueness is a §3.5 violation, not a stylistic warning).
...
**Scope**: Add the §5.2 layer 3 PreToolUse hook on `Edit`/`Write` for `personas/*.md`, `SPEC.md`, `CLAUDE.md` (HANDOFF.md excluded per §1). Hook surfaces a **summary** of the carve-out registry per the §5.2 layer 3 spec (count + one-line index per entry + pointer to the TOML file). Follows the same `jq -n '{hookSpecificOutput:{hookEventName:"PreToolUse",additionalContext:"..."}}'` shape as the existing Evidence-rule hook.
...
- **Pre-grep parenthetical exclusion.** Each §3.2 row's parenthetical exclusion clause (e.g. row 1's "(excluding the accessor pattern `sprite.img_data()`)") MUST be encoded as a pre-grep filter at the test's grep call site, NOT as a registry carve-out. Pre-exclusion is part of the grep specification — the row's grep was written to mean "the regex AND NOT the parenthetical". The registry carves out hits that ARE in the audit set but have a stable rationale; pre-exclusion removes hits the audit specification never considered violations. Implementation: drop the hit before pushing to `hits` (i.e. before `filter_carveouts` runs), so the parenthetical's authority lives at the grep site, not adjacent to the registry filter. **Granularity note**: the example below applies pre-exclusion at line-granularity (`hit.line.contains(accessor_pattern)`), which means a hypothetical line containing BOTH the retired identifier and the accessor pattern (e.g. `let x = sprite_name; let y = sprite.img_data();` in a single line) is dropped wholesale. For the SPEC.md / persona corpus this PR audits no such mixed lines exist, so the simpler line-granular form suffices; if a future audit surfaces a mixed line, register the legitimate hit explicitly via the carve-out registry rather than relaxing the pre-exclusion.
- **Carve-out registry.** `filter_carveouts(hits, registry_path)` deserializes the TOML registry (§3.5) into `Vec<CarveOut>` via `serde::Deserialize`, then drops any `Hit` whose canonicalized path matches an entry's canonicalized path AND whose `hit.line` contains the entry's `pattern` (where `Hit::line` is the matched line's *content* string, the same field used by `has_word_boundary_match` in the example test below). Both sides canonicalize before comparison so spelling variants (`SPEC.md` vs `./SPEC.md` vs `compiler/../SPEC.md`) collapse to one identity — the same canonical-form comparison the well-formedness gate's pair-uniqueness ledger uses. The filter keys on line content containing the pattern, not on a stored line number — per-file `pattern` uniqueness is what makes the per-line carve-out unambiguous, and the gate's load-time invariants (per-file uniqueness, pair uniqueness on canonical paths, repo-root containment) are asserted **at registry-load time by the well-formedness gate** (`compiler/tests/doc_invariants_carveouts_parses.rs`; the canonical contract enumeration is the gate's module docstring). The gate and `filter_carveouts` live in separate integration-test binaries (cargo runs them independently), so the coupling is CI-level: any commit failing the gate is rejected by `cargo test` before the chunk-8 cross-grep test result is consulted, and a green CI run is therefore evidence that every gate-asserted invariant holds. `filter_carveouts` may treat the registry as known-sound when run as part of a green test suite; if a chunk-8 author wants stronger isolation when running the test binary alone, they can defensively re-assert the gate's invariants on load. The registry path is a single `const CARVEOUT_REGISTRY: &str = "tests/doc_invariants_carveouts.toml";` declared once at the top of `doc_invariants.rs`. Like the search helper, the path is resolved via `crate_dir().join(CARVEOUT_REGISTRY)` so it works regardless of CWD.
    ...
    for needle in &["img_data", "sprite_name"] {
        for root in DOC_SURFACE {
            for hit in recursive_grep(Path::new(root), needle, &["rs", "md"], &["doc_invariants.rs"]) {
                // Pre-grep exclusion (§3.2 row 1's parenthetical:
                // "(excluding the accessor pattern `sprite.img_data()`)").
                // The row's grep was written to mean
                // "(\bimg_data\b|\bsprite_name\b) AND NOT sprite.img_data()" —
                // the parenthetical is part of the grep specification, so the
                // exclusion lands at the grep site, not adjacent to the
                // registry filter. The error-payload `sprite_name: ...` field
                // on `ErrorKind::SpriteNotFound` is a registry carve-out (see
                // §3.5 examples), filtered after this pre-exclusion.
                if hit.line.contains("sprite.img_data()") {
                    continue;
                }
                // Word-boundary check: distinguish the retired identifier from
                // unrelated substrings (e.g. `img_data` inside a longer ident).
...
3. **Carve-out registry location and format** → **TOML at `compiler/tests/doc_invariants_carveouts.toml` (working path); enforcement layered, not located.** Rationale (per user): location only matters if the file is forgotten; if multiple layers always surface it, the path is incidental. Three project-local layers in §5.2 (CI guard tests, CLAUDE.md source-of-truth row, PreToolUse hook). The TOML format lets the §5.1 guard tests deserialize the registry with `serde::Deserialize` rather than hand-parsing markdown; the addressing contract (per-file `pattern` uniqueness, mechanically checkable per entry) lives at §3.5.
...
- The carve-out registry (`compiler/tests/doc_invariants_carveouts.toml` per §3.5 working path) exists, every entry conforms to the scaffold's schema-by-construction (per §3.5), each entry's `pattern` satisfies the per-file uniqueness contract, and the file is referenced by every guard test through one constant.
