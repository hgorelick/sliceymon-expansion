...
| §F-series source citations referencing plan-only sections | CLAUDE.md "plans go stale once executed" — source code MUST NOT cite plan-section IDs as durable references | `rg -nE '§F[0-9]+'` (asserted surface is `compiler/src/` only, per `DOC_SURFACE` in the search-primitive subsection; `compiler/tests/` is **out of scope** for this guard test even though the historical sweep covered it) |
| `§Chunk Na` / `plan §N` / direct-plan-filename source citations | Same protected invariant as §F-N; spelling differs | `rg -nE '§Chunk [0-9]+\|plan §[0-9]+\.[0-9]+\|plan §[0-9]+\b\|parent plan §[0-9]+\|PLATFORM_FOUNDATIONS_PLAN\.md\|AUTHORING_ERGONOMICS_PLAN\.md'` (asserted surface is `compiler/src/` only, per `DOC_SURFACE` in the search-primitive subsection; `compiler/tests/` is **out of scope** for this guard test) |
