...
**Registry format** (resolved during round-1 audit; line-number-keyed addressing dropped during PR #18 implementation per the user's "line numbers are extremely fragile" decision). The registry is **TOML** at `compiler/tests/doc_invariants_carveouts.toml`. The schema is documented by construction in the scaffold's header comment (the example `[[carveout]]` block at the top of the file IS the schema, with field meanings inline as comments above each field, plus the append-discipline bullets at the bottom). The plan does not re-list the field set; the scaffold is the implementation form and the canonical authority.

The load-bearing addressing contract is `pattern`-uniqueness: every `[[carveout]]` entry's `pattern` MUST appear **exactly once** in the file at `path`. Uniqueness — not a recorded line number — locates the carved-out site. This addressing eliminates three failure modes that line-number-keyed entries had: silent collision (line-number-keyed entries silently whitelisted unrelated lines that happened to contain the pattern; under uniqueness, count >= 2 is a guard-test failure), edit-induced drift (an unrelated edit shifting line numbers tripped every entry below the edit; under uniqueness, line numbers are not stored), and pattern removal (line-number-keyed entries silently rotted when the carved-out line was deleted; under uniqueness, count == 0 is a guard-test failure). To regain uniqueness when a pattern becomes ambiguous, the author lengthens the pattern by quoting more of the carved-out line.

TOML is chosen because (a) the schema is rigid enough that the §5.1 guard tests can deserialize it with `serde::Deserialize` rather than hand-parsing markdown, (b) the pattern-uniqueness invariant is mechanically checkable per entry — `path.read().matches(&pattern).count() == 1` — and any failure (collision, removal, or ambiguity) surfaces as a guard-test failure rather than silent rot. The carve-out registry needs a TOML well-formedness gate from the moment it ships, otherwise structural regressions to the fixture (invalid TOML, broken example schema, stray top-level key) land silently — `cargo test` cannot parse a `.toml` file without the dep. So `toml = "0.8"` belongs in `[dev-dependencies]` alongside the registry scaffold, consumed first by the well-formedness gate and then by the §5.1 guard tests. Adding a dev-dep is **outside the doc-only PR boundary** (§0/§8) — it's the one exception, justified by the well-formedness gate (and, later, the §5.1 guard tests) being load-bearing for the audit's structural-enforcement story. The §8 anti-pattern "no source-code behavior changes" stands; a `[dev-dependencies]` addition does not change runtime behavior.

Examples of stable rationales (each becomes a `[[carveout]]` entry):
- "Negation, deliberate" — the word being negated is the very thing the grep matches. Canonical site: `SPEC.md` "There is no separate validator pass to bolt on later." Sibling sites: every hit of §3.2 row 7's grep (the four enumerated there). Each gets its own `[[carveout]]` entry; the rationale text is identical, the `path` and `pattern` fields differ (each `pattern` must satisfy the per-file uniqueness contract documented in the scaffold).
