...
- `compiler/src/extractor/fight_parser.rs` — 1 hit (`content.find(prefix).unwrap()`) replaced with direct offset arithmetic (`content.len() - after_template.len() + 1`). The old `.unwrap()` was structurally unreachable under the existing `is_double || is_single` guard, so for every realistic fight template (registry-style identifiers like `CS_Quagmire`, `tann_goodbye` — no template in `working-mods/*.txt` embeds `.((` or `.(`) the refactor is a pure panic-removal. For *pathological* templates whose bytes embed `.((` or `.(` the old code would lock onto the in-template match and return a mid-template offset, so the refactor is strictly more correct in that edge case — not just panic-removal. The in-code comment on `fight_parser_malformed_propagates_error` is the authoritative statement (line numbers drift; search by name).
