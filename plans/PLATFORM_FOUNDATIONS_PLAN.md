...
1. `util::slice_before_chain_and_cast(body: &str) -> &str` — returns the longest prefix of `body` that precedes the first depth-0 occurrence of any marker in the canonical set **§F10-MARKERS** = {`.i.`, `.sticker.`, `.cast.`, `.abilitydata.`}. When no such marker exists at depth 0, returns the full slice. Callers (`parse_simple` / `parse_with_ability` / `parse_legendary`) feed this slice to the scalar extractors for `hp` / `color` / `sd` / `img`. Correctness follows from the emitters' field order: top-level scalars are always emitted before the chain and cast blocks. `.abilitydata.` is in the set because the Capture-with-ability parser reaches the outer cast region via `extract_nested_prop(modifier, ".abilitydata.")` at depth 0; keeping it in **§F10-MARKERS** means the slice excludes the Capture cast effect even though no legit `.abilitydata.` token lives there under the emitter's actual shape.
2. `util::extract_color(content, depth_aware)` — add the `depth_aware: bool` flag already present on `extract_hp` / `extract_sd`. Route per container shape: `parse_legendary` passes `depth_aware = true` (its scalar prefix is genuinely flat at depth 0, so a depth-1 `.col.` is always chain / cast drift); `parse_simple` and `parse_with_ability` pass `depth_aware = false` because legit Capture scalars live at depth ≥ 2 inside `((hat.replica...))` / `((hat.(replica...)))` and `depth_aware = true` would skip them. Correctness for the Capture leak class is carried entirely by the slice — **§F10-MARKERS** includes `.abilitydata.`, so the slice ends before the cast effect's interior `.col.X` ever enters the scan.
...
**Emission-order co-requirement**: §F10 depends on emitters placing every top-level scalar field (`.col.`, `.hp.`, `.sd.`, `.img.`) *before* the chain / cast region. `emit_simple` / `emit_with_ability` / `emit_legendary` in `compiler/src/builder/replica_item_emitter.rs` all emit `.col.` / `.hp.` before the chain already; `.sd.` and `.img.` must precede the chain too. This chunk reorders those in all three emitters — without the reorder, an emit→parse cycle where the chain carries a decoy `.sd.` / `.img.` and the real value comes *after* the chain would silently lose the real value to first-match.

**Structural check (per the chunk-impl hook)**: the two tools are not parallel representations. `slice_before_chain_and_cast` narrows *where* to scan; the `depth_aware` flag narrows *how* to scan within the slice. They compose, not duplicate. For Captures, (a) a top-level-absent scalar whose decoy sits inside the chain / cast needs **only** the slice (its `.i.` / `.sticker.` / `.cast.` / `.abilitydata.` membership in **§F10-MARKERS** guarantees it's excluded), and `depth_aware = true` is actively wrong (skips legit scalars at depth ≥ 2). For Legendary, the scalar prefix is flat at depth 0, so the flag lets the scan reject any stray depth-1 match that later code evolution might introduce (defensive — current bodies never exercise it). Either mechanism alone fails the other container shape's invariant, so both ship; they do not duplicate because they guard different container shapes' leak surfaces.
...
- In `parse_simple`, `parse_with_ability`, `parse_legendary`, compute `let scalar_slice = util::slice_before_chain_and_cast(body_or_modifier);` once, then route every scalar extractor through that slice. `depth_aware` is shape-dependent, not uniform — see §F10 "Fix" item 2: `parse_legendary` passes `depth_aware = true`; `parse_simple` and `parse_with_ability` pass `depth_aware = false` because their legit scalars live at depth ≥ 2. `extract_img_data(scalar_slice)` is always depth-aware (uses `find_last_at_depth0` under the hood).
...
- [ ] `extractor::capture_color_ignores_cast_interior` — analogous for `parse_with_ability` with an outer depth-0 `.abilitydata.(...)` carrying `.col.X` at depth ≥ 2. The slice trims at `.abilitydata.` (member of **§F10-MARKERS**), so the decoy never enters the scan. `parse_with_ability` uses `depth_aware = false` on the resulting slice — `depth_aware = true` would skip the legit depth ≥ 2 Capture scalars.
...
- [x] Replica parsers (`parse_simple` / `parse_with_ability` / `parse_legendary`) route scalar extraction through `util::slice_before_chain_and_cast`; chain-interior `.hp.` / `.col.` / `.sd.` / `.img.` substrings in sidesc / cast-effect / enchant text do not leak into top-level fields. `depth_aware` is shape-dependent: `parse_legendary` passes `true` (flat depth-0 scalar prefix); `parse_simple` / `parse_with_ability` pass `false` (legit scalars live at depth ≥ 2 inside the `((hat.replica...))` wrap). The emitter reorders `.sd.` / `.img.` before the chain in all three emit paths, per §F10's emission-order co-requirement. (Chunk 9, §F10, 2026-04-22.)
