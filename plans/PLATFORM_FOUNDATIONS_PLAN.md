...
1. `util::slice_before_chain_and_cast(body: &str) -> &str` — returns the longest prefix of `body` that precedes the first depth-0 occurrence of any marker in the canonical set **§F10-MARKERS** = {`.i.`, `.sticker.`, `.cast.`}. When no such marker exists at depth 0, returns the full slice. Correctness follows from the emitters' field order: top-level scalars are always emitted before the chain and cast blocks.
