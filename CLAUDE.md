...
- Plans are not sources of truth — once a plan is executed, the code becomes the truth. Don't cite plans anywhere outside `plans/` itself; cite the underlying authority (SPEC §, corpus bytes, a test ID, or a PR-anchored tribunal-round reference) directly. The rule's universe covers in-tree prose (source comments, tests, specs, docs, configs) **and** any durable, project-authored prose on GitHub — PR titles, PR descriptions, PR review summaries and inline review comments, issue bodies and comments, commit messages, release notes, annotated tag messages, GitHub Discussions posts and replies, wiki pages, and project-owned gists. The enumeration is non-exhaustive: any GitHub-hosted surface that persists across the plan's rename or deletion is in scope, since each resolves against the same stale plan-filename handle. Bare `Chunk Na` references for **landed** chunks are carved out — chunk identity is durable in git history once the chunk lands. References to **in-flight** chunks (where the only resolution path is the plan filename) are NOT carved out: describe the future work without naming the chunk identifier (e.g. "the future real parser surfaces…", "a planned typed-sum rewrite replaces…"), since naming an unlanded chunk couples the comment to a plan that will be deleted or renamed when the chunk lands. Tribunal-round references in source comments and doc prose must be PR-anchored (`PR #14 round-9 merge contract`, not bare `round-9 merge contract`) — bare `round-N` resolves to nothing without conversational context, the same stale-handle failure mode as a plan-filename citation; PR-anchored form lands on a specific GitHub review thread that is durable on the platform. A single anchor per comment block is sufficient when every round reference in that block belongs to the same PR (e.g. one parenthetical "(per PR #14: round-9 merge contract at merge.rs:106-111, round-12 add contract above)" rather than re-prefixing each ref); cross-PR blocks must anchor each ref separately. **Carve-out — describing what was edited in a plan file**: identifying the plan file by its literal repo-relative path (e.g. `plans/FOO_PLAN.md`) when the surrounding prose describes a change made *to* that file in this PR is licensed; the plan filename is then a description-of-edit, not a content citation, and naming the file you're editing is unavoidable. Descriptive paraphrases that resolve to a specific plan ("the active doc-invariant audit plan") are NOT carved out — they create the same stale-handle failure mode while obscuring which file was meant; use the literal filename when describing edits, or omit the plan reference entirely when describing the work itself.
